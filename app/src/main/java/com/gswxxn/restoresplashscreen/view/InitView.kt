...
class InitView(private val datalist: HashMap<String, ItemData>) {
    ...
    inner class ItemData(val title: String, val hideMenu: Boolean) {
        ...
        fun GetDataBinding(
            defValue: () -> Any,
            recvCallbacks: (View, Int, Any) -> Unit
        ): DataBinding.BindingData {
        ...
        fun SeekBar(
            ...
            max: Int,
            defaultProgress: Int,
            dataSend: DataBinding.Binding.Send? = null,
            dataBindingRecv: DataBinding.Binding.Recv? = null,
            callBacks: ((Int, TextView) -> Unit)? = null
        ) {
            itemList.add(
                SeekBarV(
                    key,
        ...
        fun TextS(
            ...
            itemList.add(
                TextWithSwitchV(
                    TextV(text, textId),
                    SwitchV(key, onClickListener = onClickListener),
                    dataBindingRecv
                )
            )
        }
        ...
        fun RadioView(
            key: String,
            dataBindingRecv: DataBinding.Binding.Recv? = null,
            data: RadioViewV.RadioData.() -> Unit
        ) {
    ...
    fun register(key: String, title: String, hideMenu: Boolean, itemData: ItemData.() -> Unit) {
        ...
        ) throw Throwable("[$key] This is reserved and cannot be used!")
